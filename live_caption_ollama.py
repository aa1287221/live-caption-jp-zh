...
# 完全沒停頓、一直講很久的狀況下才會強制切斷。之前拉到 25 秒讓字幕感覺變超慢，
# 調回一個平衡點——多數句子本來就會在幾秒內遇到真正的停頓，很少真的撐到這個上限
...
def pick_window() -> tuple[int, str, int]:
    """跳出視窗清單，讓你選擇要擷取哪一個視窗，回傳 (視窗控制代碼, 標題, 所屬程式 PID)"""
    ...
    tk.Label(
        root, text="請選擇要擷取畫面的視窗（通常是你的瀏覽器）：",
        font=("Microsoft JhengHei", 11),
    ).pack(pady=10)

    listbox = tk.Listbox(root, font=("Microsoft JhengHei", 11))
    ...
    def confirm():
        ...
        picked["hwnd"] = chosen._hWnd
        picked["title"] = chosen.title
        root.destroy()

    tk.Button(
        root, text="確認選擇", font=("Microsoft JhengHei", 11), command=confirm,
    ).pack(pady=10)
    ...
    if "hwnd" not in picked:
        print("沒有選擇視窗，結束程式。")
        sys.exit(0)

    hwnd = picked["hwnd"]
    _, pid = win32process.GetWindowThreadProcessId(hwnd)
    return hwnd, picked["title"], pid
...
def pick_input_device(pa: "pyaudio.PyAudio") -> dict | None:
    """列出可用的 WASAPI loopback 擷取來源，讓你選一個。

    如果你有裝虛擬音訊線（例如 VB-CABLE），並且已經在 Windows 音量混音器裡
    把瀏覽器的輸出裝置設定成「CABLE Input」，這裡要選「CABLE ... [Loopback]」——
    這樣抓到的會是瀏覽器乾淨、全音量的原始聲音，完全不會被真人聽到，
    也就不需要再調低音量了。

    不選（直接 Enter）的話會用系統目前的預設輸出裝置，這種狀況下程式會自動
    調低來源音量，但沒辦法做到完全沒有背景音（見檔案開頭的說明）。
    """
    try:
    ...
    cable_hint = next((d for d in devices if "CABLE" in d["name"].upper()), None)

    print("\n選一個要擷取聲音的來源：")
    if cable_hint:
        print("  （偵測到虛擬音訊線，如果瀏覽器輸出已經設定成 CABLE Input，選下面的 CABLE 那項）")
    for n, d in enumerate(devices):
        print(f"  {n}: {d['name']}")

    choice = input("輸入編號選擇（直接 Enter 用系統預設輸出裝置）：").strip()
    if not choice:
        return None
    try:
        return devices[int(choice)]
    except (ValueError, IndexError):
        print("看不懂這個編號，使用系統預設。")
        return None
...
def pick_output_device(exclude_name: str | None = None) -> int | None:
    """列出可以用來播放延遲聲音的輸出裝置，讓你選一個（建議選耳機）。

    一定要跟「被擷取聲音的來源裝置」分開，不然我們自己播出來的延遲聲音會被
    系統錄音功能聽到、寫回緩衝區、又再播一次，變成回音疊加的無限循環
    （這正是「聲音瘋狂重複」的原因）。
    回傳選到的裝置編號；沒選、或找不到裝置就回傳 None（會退回系統預設，
    但退回預設的話有很高機率又出現回音問題）。
    """
    try:
        ...
        if not candidates:
            print("找不到可用的輸出裝置清單，將使用系統預設（可能會有回音問題）。")
            return None

        print("\n選一個裝置來播放延遲後的聲音（建議選耳機類，跟擷取聲音的來源分開）：")
        for n, (idx, name) in enumerate(candidates):
            warn = "　←目前的擷取來源，選這個會回音！" if exclude_name and exclude_name in name else ""
            print(f"  {n}: {name}{warn}")

        choice = input("輸入編號選擇（直接 Enter 用系統預設，可能會回音）：").strip()
        if not choice:
            return None
        try:
            return candidates[int(choice)][0]
        except (ValueError, IndexError):
            print("看不懂這個編號，改用系統預設（可能會回音）。")
            return None
    except Exception as e:
        print(f"列出音訊裝置失敗，改用系統預設：{e}")
        return None
...
class DelayedAudioPlayer:
    ...
    def _feeder_loop(self, out_sr: int):
        ...
        chunk_samples_in = max(1, int(0.5 * TARGET_SR))
        # 拉大一點緩衝目標：真正執行時這條背景執行緒要跟畫面擷取/Whisper/翻譯
        # 一起搶執行資源，比單獨測試時容易被排擠、來不及補資料，緩衝大一點
        # 才有本錢撐過這種被搶資源的空檔，不然 callback 只能吐靜音
        ...
        while not self._feeder_stop.is_set():
            try:
                ...
                next_read_time += chunk_samples_in / TARGET_SR
                # 這段是真的要播給你聽的，用高品質重取樣，不要用 resample_linear
                # （那個給 Whisper 辨識用可以，人耳朵聽得出差別）
                resampled = resample_high_quality(samples, TARGET_SR, out_sr)
    ...
    def run(self):
        ...
        while not self._stop.is_set():
            ...
            if self.audio_ring is not None:
                self.audio_ring.write(audio_16k)
            ...
            while len(pending) >= VAD_CHUNK_SAMPLES:
                ...
                if speech_active and (is_end or force_flush):
                    ...
                    # 記下這句話「講完的當下」時間，之後 DISPLAY_DELAY_SEC 要拿來算
                    # 字幕該幾點幾分顯示，才能對上延遲播放畫面裡這個人開口的瞬間
                    self.utterance_queue.put((utterance, time.time()))
                    utterance_frames = []
                    utterance_ms = 0
...
class Worker(threading.Thread):
    ...
    def run(self):
        while not self._stop.is_set():
            ...
            # 這句話該在幾點幾分顯示：講完的當下 + 延遲播放秒數，跟畫面/聲音用同一個延遲值，
...
class PlayerWindow:
    ...
    def _poll_captions(self):
        ...
        now = time.time()
        ready = [item for item in self._pending_caption if item[0] <= now]
        if ready:
            self._pending_caption = [item for item in self._pending_caption if item[0] > now]
            _, ja_text, zh = ready[-1]  # 只顯示最新一句，中間來不及顯示的就跳過
...
def main():
    ...
    hwnd, window_title, pid = pick_window()
    print(f"已選擇視窗：{window_title}")

    episode_title = input("這次要看的節目/集數名稱（可留空按 Enter 跳過）：").strip()

    delay_input = input(
        f"要延遲幾秒鐘播放（讓字幕來得及跟上，直接按 Enter 用預設 {DISPLAY_DELAY_SEC:.0f} 秒）："
    ).strip()
    if delay_input:
        try:
            DISPLAY_DELAY_SEC = max(1.0, float(delay_input))
        except ValueError:
            print("看不懂這個數字，使用預設延遲。")
    print(f"延遲播放：{DISPLAY_DELAY_SEC:.1f} 秒")

    _pa_probe = pyaudio.PyAudio()
    input_device = pick_input_device(_pa_probe)
    _pa_probe.terminate()
    ...
    if using_virtual_cable:
        ...
        # 一定要擋住流程、等你確認重新整理過頁面，才開始擷取：切換裝置的指令
        # 不會馬上生效在「已經在播放」的舊工作階段上，如果沒重新整理就開始擷取，
        # 舊的工作階段還留在原本裝置直接出聲，會跟延遲播放器同時搶著響、疊在一起
        # 變成很吵的雜音，直到你之後重新整理頁面才會停止——先擋住就不會有這段
        input("\n請去瀏覽器重新整理影片頁面（或重新開始播放），完成後按 Enter 繼續：")
    ...
    audio_ring = AudioRingBuffer(sample_rate=TARGET_SR, max_seconds=buffer_seconds)
    ...
    output_device = pick_output_device(exclude_name=audio_capture.device.get("name"))
    ...
    root = tk.Tk()
    PlayerWindow(root, frame_buffer, caption_queue, episode_title)

    # 逐字稿是獨立視窗，預設貼在螢幕右側，跟播放視窗分開、比例各自看得順眼；
    # 兩個都是一般可拖曳縮放的視窗，看你想怎麼擺都可以自己調整
